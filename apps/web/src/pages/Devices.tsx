...
const Devices = () => {
  ...
  const handleDelete = async (friendlyName: string) => {
    try {
      ...
      window.dispatchEvent(new CustomEvent('app_notification', {
        detail: { type: 'warning', title: 'Device Removed', description: `${friendlyName} has been deleted from the database.` }
      }));
  ...
  const handleRename = async (oldName: string, updatedName: string) => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('app_notification', {
        detail: { type: 'info', title: 'Device Renamed', description: `${oldName} was renamed to ${finalName}.` }
      }));
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <GlassCard className="p-2 lg:p-3 w-full lg:w-[220px] shrink-0 flex flex-row lg:flex-col gap-2 overflow-x-auto no-scrollbar items-center lg:items-stretch">
          {displayRooms.map((r) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="truncate max-w-[120px] lg:max-w-[130px] text-left">{r.name}</span>
              </div>
          ...
          <button
            ...
            className="shrink-0 flex items-center justify-start gap-2 rounded-2xl px-4 py-3 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors lg:w-full"
          >
        ...
        <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-[1fr_320px] xl:grid-cols-[1fr_380px] gap-6 items-start">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-[repeat(auto-fill,minmax(160px,1fr))] gap-4 content-start">
            {isLoadingDevices || isLoadingRooms ? (
              ...
              <p className="col-span-full text-center py-10 text-muted-foreground">No devices found in this room.</p>
            ) : (
          ...
          {selectedDevice ? (
            ...
            <GlassCard className="hidden md:flex p-6 text-sm text-muted-foreground items-center justify-center h-fit w-full">
